...
public final class GameResumeActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (game != null) {
            Belote.getBeloteFacade(this).getGameLock().readLock().lock();
            try {
                ...
                Belote.getBeloteFacade(this).getGameLock().readLock().unlock();
            }
        }
    }
    ...
    private void loadSavedValues(Bundle savedInstanceState) {
        if (savedInstanceState != null) {
            isShowWinner = savedInstanceState.getBoolean(showWinnerStr);
        }
    }
    ...
    private TableLayout getGameInfo(TextDecorator textDecorator) {
        ...
        if (announce != null) {
            final boolean ntAnnounce = announce.getAnnounceSuit().equals(AnnounceSuits.NotTrump);

            TableLayout table = new TableLayout(this);
            // Draw team player' sequences.

            for (final Team team : game.teams()) {
                if (team.equals(Team.E_W)) {
                    TableRow fake = createFakeRow(dip4, dip4);
                    table.addView(fake);
                }
            ...
            TableRow fake = createFakeRow(dip5, dip5);
            table.addView(fake);
            ...
            for (final Team team : game.teams()) {
                row = creatPointsDistributionTeamRow(team);
                table.addView(row);
            }
    ...
    private String getTeamCaption(Team team) {
        ...
        for (final Player player: team.players()) {
            PlayerNameDecorator playerName = new PlayerNameDecorator(player);
            sentence.addWord(playerName.decorate(this));
        }
    ...
    private TableRow createCouplesRow(Team team) {
        ...
        llp.rightMargin = Belote.fromPixelToDip(this, 10);
    ...
    private TableRow createAnnouncePointsRow(Team team, TextDecorator textDecorator) {
        ...
        PictureDecorator pictureDecorator = new PictureDecorator(this);
        for (final Player player: team.players()) {
            Iterable<Sequence> sequencesList = player.getCards().getSequencesList().list();
    ...
    private TableRow createFakeRow(int topMargin, int bottomMargin) {
    ...
    private void setFaceView() {
        ...
        String message;
        Player human = HumanBeloteFacade.HUMAN_PLAYER;
        ...
        if (Team.N_S.equals(game.getWinnerTeam())) {
        ...
        team0.setText(String.valueOf(Team.N_S.getPoints().getAllPoints()));
        ...
        if (Team.E_W.equals(game.getWinnerTeam())) {
        ...
        team1.setText(String.valueOf(Team.E_W.getPoints().getAllPoints()));
    ...
    private TableRow createRow(String caption) {
        ...
        TableLayout.LayoutParams tableParams = new TableLayout.LayoutParams();
        tableParams.bottomMargin = Belote.fromPixelToDip(this, 2);
        row.setLayoutParams(tableParams);
